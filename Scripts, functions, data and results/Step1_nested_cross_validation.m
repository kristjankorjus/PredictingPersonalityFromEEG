  ...
  for iRoi = Roi
    ...
    for iFreq = Freq
      ...
      elseif iFreq == 2
        ...
        % Initialize
        DataMatrix = zeros(num_of_samples,NumOfChannels*98);
      ...
      % Bands based on literature  
        ...
        for ii = 1:num_of_samples
          for jj = 1:NumOfChannels
            ...
            % superhigh gamma
            DataMatrix(ii,(jj-1)*11+11)=mean(Data1(ii,jj,360:398));
          end
        end
      end
...
Parameters = zeros(5,size(Results,2));
for ii = 1:5
